...
namespace DAL
{
    ...
    public class dalOrderState
    {
        ...
        public static ENTITY.OrderState getSomeOrderState(int stateId)
        {
            ...
            ENTITY.OrderState orderState = null;
            /*如果查询存在记录，就包装到对象中返回*/
            if (DataRead.Read())
            {
                orderState = new ENTITY.OrderState();
        ...
        /*查询订单状态*/
        public static DataSet GetOrderState(string strWhere)
        {
            try
            {
                string strSql = "select * from OrderState" + strWhere + " order by stateId asc";
